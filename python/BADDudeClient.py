...
class BADDudeClient:
    ...
    def tpiEnter(self):
        self._writeInt8(BADDUDE_CMD_TPIENTER)
        self._pollAck()

    def tpiExit(self):
        self._writeInt8(BADDUDE_CMD_TPIEXIT)
        self._pollAck()

    def flashPrime(self):
        self._writeInt8(BADDUDE_CMD_FPRIME)
        self._pollAck()

    def flashErase(self):
        self._writeInt8(BADDUDE_CMD_FERASE)
        self._pollAck()

    def flashWrite(self, bytes: bytes, progressCallback: Callable[[float], None]):
        self._writeInt8(BADDUDE_CMD_FWRITE)
        ...
        while True:
            if (self._readInt8() == BADDUDE_CMD_FWPROG):
    ...
    def _pollAck(self):
        while True:
            receivedByte = self._readInt8()
            if receivedByte == BADDUDE_CMD_ACK:
                return
            elif receivedByte == BADDUDE_CMD_NACK:
    ...
    def _writeBytes(self, bytes: bytes, chunked: bool):
        if not chunked:
            ...
            for i in range(0, len(bytes), BADDUDE_MAX_CHUNK_SIZE):
                ...
                if self._readInt8() != BADDUDE_CMD_NEXTCHUNK:
