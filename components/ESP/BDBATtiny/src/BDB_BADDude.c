...
void BADDude_loop(){
    ...
    switch (opcode){
    case BADDUDE_CMD_PING:
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_TPIENTER:
        TPI_PHY_init();
        TPI_enter();
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_TPIEXIT:
        TPI_disableNVMController();
        TPI_exit(true);
        TPI_PHY_deinit(false);
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_FPRIME:
        TPI_enableNVMController();
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_FERASE:
        TPI_NVM_ChipErase();
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_FWRITE:
        {
            uint32_t size = 0;
    ...
    case BADDUDE_CMD_FRead:
        ...
        break;
    case BADDUDE_CMD_ATRESET:
        BDB_ATtiny_Reset();
        BADDUDE_acknowledge();
        break;
    case BADDUDE_CMD_CPCNT:
        {
            BADDUDE_acknowledge();
    ...
    case BADDUDE_CMD_CPRUN:
        ...
        break;
    case BADDUDE_CMD_SREAD:
        {
            BADDUDE_acknowledge();
            uint8_t* buf = malloc(BADDUDE_MAX_CHUNK_SIZE);

            for(int i = 0; i < ATTINY_MEMORY_SRAM_SIZE; i += BADDUDE_MAX_CHUNK_SIZE){
                TPI_readSRAM(buf,ATTINY_MEMORY_SRAM_ADDR+i,BADDUDE_MAX_CHUNK_SIZE);
                if (i != 0)
                    BADDUE_pollNextChunk();
                usb_serial_jtag_write_bytes(buf,BADDUDE_MAX_CHUNK_SIZE,portMAX_DELAY);
            }
